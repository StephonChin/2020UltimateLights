...
void sys_wifi_conneting_router(void){
	static u32 _cnn_time = 0;
	static wl_status_t old_status = WiFi.status(); 
	wl_status_t status  = WiFi.status();

	if(_cnn_time > 0 && DIFF(_cnn_time, m2m_current_time_get() ) > (60 * 1000) ){
		_cnn_time = 0;
	...
	case WL_SCAN_COMPLETED:
		_cnn_time = m2m_current_time_get();
		break;

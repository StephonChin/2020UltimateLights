...
static __inline__ Write_Low(void);
static __inline__ Write_High(void);
...
void Ws2812_Init(void)
{
  ...
  //clear the buffer
  for(j=0; j<NUM_I2S_PIXEL_WORDS1; j++)    i2s_pixels_buffer1[j] = 0xffffffff;
  for(j=0; j<NUM_I2S_PIXEL_WORDS2; j++)    i2s_pixels_buffer2[j] = 0xffffffff;
...
void Ws2812_Show(void)
{
  ...
  *i2s_word_ptr = 0;
  ...
  while (!brkflag){
    ...
    Write_Low();
    Write_Low();
    if (first == 1 && (i2s_word_ptr >= (i2s_buffer2 + NUM_I2S_PIXEL_WORDS2 - 8)))   brkflag = true;
  }
  ...
  i2s_word_ptr++;
  while(i2s_word_ptr < i2s_buffer2 + NUM_I2S_PIXEL_WORDS2)  *i2s_word_ptr++ = 0x0;
